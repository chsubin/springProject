 ...
 <script>
 	'use strict';
	  ...
	  function selectDelCheck() {
	  	...
	  	if(delItems=="") myform.chk.value="/";
			
	  	$.ajax({
	  		type : "post",
	  		url  : "${ctp}/admin/extra/fileSelectDelete",
	...
	<nav class="navbar navbar-expand-sm border" style="background-color:#fff">
	  <ul class="navbar-nav">
	    <li class="nav-item active">
	      <a class="nav-link" href="#"><h4 class="p-0 m-0">CKEDITOR</h4></a>
	    </li>
	    <li class="nav-item">
	      <a class="nav-link" href="adminTempFileList">| TEMP(리뷰 다운로드)</a>
	    </li>
	  </ul>
	</nav>
						...
						<tr class="text-center">
							...
							<td>
								<c:if test="${fn:contains(file,'.')}">
									<label for="chk${file}" ><img src="${ctp}/data/ckeditor/${file}" style="width:50px;height:50px"></label>
								</c:if>
								<c:if test="${!fn:contains(file,'.')}">
