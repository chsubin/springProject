...
<div class="container-fluid  p-5">
	<nav class="navbar navbar-expand-sm border" style="background-color:#fff">
	  <ul class="navbar-nav">
	    ...
	    <li class="nav-item">
	      <a class="nav-link" href="adminUpdateProduct"><h4 class="p-0 m-0"> 상품수정 </h4></a>
	    </li>
	    <li class="nav-item">
		...
		<form name="myform" method="post" enctype="multipart/form-data" >
			<table class="table">
				...
				<tr>
					...
					<td><input type="number" name="deliveryPrice" id="deliveryPrice" value="${vo.deliveryPrice}" class="form-control"></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>
						<c:forEach var="optionVo" items="${optionVos}" varStatus="st">
							<span id="optionIdx${st.count}"><c:if test="${st.count!=1}"><br/></c:if><b>옵션${st.count} | </b> 옵션이름 : <input class="form-control" style="display:inline;width:30%" type="text" name="optionName" id="optionName${st.count}" value="${optionVo.optionName}"> , 
							옵션가격 : <input class="form-control" style="display:inline;width:30%" type="text" name="optionPrice" id="optionPrice${st.count}"  value="${optionVo.optionPrice}"></span>
						</c:forEach> 
						<span id="options"></span>
						<span id="optionsBtn">
							<span class="btn btn-outline-info" onclick="optionPlus(${optionNum})"> + </span><c:if test="${optionNum!=1}"><span class="btn btn-outline-danger" onclick="optionDelete(${optionNum})"> - </span></c:if>
						</span>	
					</td>
				</tr>
				<tr>
